###----------------------------###
#- Title: Function for converting raw PI data to live cover
...
fx_SummarizePI_Live <- function(data) {
  ...
  #--Fill in matrix of unique live species at each point with 1=True and 0=False---
  for (i in 1:length(sppobs)){
    sppmatrix[,i] = ifelse(((data$top == colnames(sppmatrix)[i]) & (data$alive == TRUE)) | 
                             ((data$surface == colnames(sppmatrix)[i]) & (data$surface_alive == TRUE)) | 
                             ((data$lcs1 == colnames(sppmatrix)[i]) & (data$lca1 == TRUE)) | 
                             ((data$lcs2 == colnames(sppmatrix)[i]) & (data$lca2 == TRUE)) | 
                             ((data$lcs3 == colnames(sppmatrix)[i]) & (data$lca3 == TRUE)) | 
                             ((data$lcs4 == colnames(sppmatrix)[i]) & (data$lca4 == TRUE)) | 
                             ((data$lcs5 == colnames(sppmatrix)[i]) & (data$lca5 == TRUE)) | 
                             ((data$lcs6 == colnames(sppmatrix)[i]) & (data$lca6 == TRUE)) | 
                             ((data$lcs7 == colnames(sppmatrix)[i]) & (data$lca7 == TRUE)) | 
                             ((data$lcs8 == colnames(sppmatrix)[i]) & (data$lca8 == TRUE)) | 
                             ((data$lcs9 == colnames(sppmatrix)[i]) & (data$lca9 == TRUE)) | 
                             ((data$lcs10 == colnames(sppmatrix)[i]) & (data$lca10 == TRUE)), 1, 0)
  }
